...
`},c=/^CREATE\s+(?:UNIQUE\s+)?(TABLE|INDEX|VIEW)(?:\s+IF\s+NOT\s+EXISTS)?\s+([A-Za-z_][A-Za-z_0-9]*)/i,l={TABLE:`tabela`,INDEX:`indice`,VIEW:`vista`},u=/^\s{2,}([a-z_][a-z_0-9]*)\s+\S/;function d(e){let t=0;for(;t<e.length;){let n=e.charAt(t);if(n===`'`||n===`"`){for(t++;t<e.length&&e.charAt(t)!==n;)t++;t++;continue}if(n===`-`&&e.charAt(t+1)===`-`)return e.slice(t+2).trim();t++}}function f(e){let t=e.split(`

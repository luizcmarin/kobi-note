import{S as e,at as t,ct as n,ft as r,h as i,it as a,u as o,ut as s,x as c}from"./index-DtxwN5g2.js";import{ALVOS_MIN as l,TIPOS as u,emojiDoTipo as d,excluirSessao as ee,horasEMinutos as f,listarSessoes as p,ordenar as m,perolaComoAnotacao as h,perolaNoCaderno as g,relogio as _,rotuloDoTipo as v,salvarSessao as y,totalDeMinutos as b}from"./dados-DpMyNUKi.js";var x=90,S=2*Math.PI*x,C=`inicio`,w=`pessoal`,T=``,E=30,D=!1,O=0,k=0,A=0,j,M=``,N=null,P=!1,F=!1,I=[],L=!1,R=!1,z=null;async function B(){I=await p(),a()}function V(){L||R||z!==null||(R=!0,(async()=>{try{await B(),L=!0}catch(t){console.error(`estudo: a carga falhou.`,t),z=e(t)}finally{R=!1,a()}})())}function H(){z=null,V(),a()}function U(){D=!0,A=Date.now(),clearInterval(j),j=setInterval(()=>{O=k+Math.floor((Date.now()-A)/1e3),a()},250),a()}function W(){D&&(k+=Math.floor((Date.now()-A)/1e3),O=k,D=!1,clearInterval(j),j=void 0,a())}function G(){let e=E*60;return e<=0?0:Math.min(1,O/e)}function K(){return Math.max(0,Math.round(O/60))}function q(){C=`oracao`,a()}function J(){O=0,k=0,M=``,N=null,P=!1,F=!1,C=`cronometro`,U()}function Y(){W(),C=`perola`,a()}async function X(){let e={...N?.id===void 0?{}:{id:N.id},assunto:T===``?n.estudo.semAssunto:T,tipo_ciclo:w,duracao_minutos:K(),concluido_em:Date.now(),nota_meditacao:M,anotacao_criada:+!!P},t=await y(e);return N={...e,id:t},await B(),N}async function Z(){N===null&&await X(),T=``,C=`inicio`,t(`estudo/historico`)}function Q(){clearInterval(j),j=void 0,D=!1,T=``,O=0,k=0,C=`inicio`,a()}function $(){return r`
  ...
  `}function ne(){let e=G(),t=O>=E*60;return r`
  ...
  `}function re(){return r`
    ...
    <kk-textarea
      ...
      .value=${M}
      @kk-input=${e=>{M=e.target.value,a()}}
    ></kk-textarea>

    <div class="editor__acoes">
      <kk-button
        ?disabled=${M.trim()===``||P}
        @click=${async()=>{let e=N??await X();await h({...e,nota_meditacao:M}),P=!0,o(n.estudo.viraAnotacaoFeito),await B()}}
      >
      ...
      <kk-button
        ?disabled=${M.trim()===``||F}
        @click=${async()=>{let e=N??await X();await g({...e,nota_meditacao:M}),F=!0,o(n.estudo.vaiCadernoFeito),a()}}
      >
  ...
  `}function ie(){let e=m(I),t=b(I);return r`
          ...
          <div class="cartoes cartoes--duas">
            ${e.map(e=>r`
                <div class="cartao cartao--parado">
                  ...
                  ${e.nota_meditacao===``?s:r`<p class="cartao__texto">${e.nota_meditacao}</p>`}

                  <span class="cartao__rodape">
                    ${e.nota_meditacao.trim()===``||e.anotacao_criada===1?s:r`
                          <kk-button
                            size="small"

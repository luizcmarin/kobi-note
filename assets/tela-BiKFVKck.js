      ...
      <div class="crono__cartao">
        ...
        ${t?r`
              <div class="crono__detalhe">
                ${e.resumo===``?i:r`<p>${e.resumo}</p>`}
                ${e.referencia===``?i:r`<p class="crono__referencia">${e.referencia}</p>`}
                ${e.link_fonte===``?i:r`
  ...
  `}var H={voltarPara(){return`home`},conteudo(e){if(k(),D!==null)return o(D,A);if(!T)return r`<div class="carregando"><kk-spinner></kk-spinner></div>`;let t=h(m(v,y,b));return t.length>0&&(queueMicrotask(L),queueMicrotask(P)),r`
      ...
      <div class="chips crono__chips">
        ${Object.entries(u).map(([e,t])=>r`
          ...
          `)}
        <button class="chip" title=${n.cronologia.limpar} @click=${z}>
      ...
      ${t.length===0?r`
            <div class="vazio">
              ...
              <p>${v.length===0?n.cronologia.vazio:n.cronologia.semFiltro}</p>
            </div>

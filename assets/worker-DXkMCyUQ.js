...
-- ── O que o Perfex tem e o Flow ganhou em 23/09/2026 ────────────────────────
...
`)){let t=a.exec(e)?.[1];if(t===void 0)continue;d.push(t);let n=o(e);n!==void 0&&n!==``&&(u[t]=n)}n.push({nome:l[2]??``,tipo:i[(l[1]??``).toUpperCase()]??`indice`,sql:t,nota:e,colunas:u,campos:d})}return n}const l=/* @__PURE__ */ new Map;function u(e){let t=l.get(e);return t===void 0&&(t=c(n[e]),l.set(e,t)),t}const d=[`note`,`flow`],f=/* @__PURE__ */ new Map;function p(){let e=[...u(`nucleo`)],t=new Set(e.map(e=>e.nome));for(let n of d)for(let r of u(n))t.has(r.nome)||(t.add(r.nome),f.set(r.nome,n),e.push(r));return e}let m;function h(e){return e===`nucleo`?(m??=p(),m):u(e)}function ee(e){return h(e).map(e=>e.sql)}function te(e){return h(e).filter(e=>e.tipo===`tabela`).map(e=>e.nome)}const g=`bioma_carimbos`,ne=[`id`,`id_global`],re=`CREATE INDEX IF NOT EXISTS idx_bioma_carimbos_hlc
  ON ${g} (hlc);`;function _(e,t){return`hlc_${e}_${t}`}function ie(e,t,n){return`('${e}', ${n}.id_global, '${t}', bio_hlc(), ${n}.${t})`}const v=`ON CONFLICT (tabela, id_global, campo)
    DO UPDATE SET hlc = excluded.hlc, valor = excluded.valor`;function ae(e){let t=e.nome,n=e.campos.filter(e=>!ne.includes(e)),r=n.map(e=>`    ${ie(t,e,`new`)}`).join(`,
`),i=n.map(e=>`  INSERT INTO ${g} (tabela, id_global, campo, hlc, valor)\n    SELECT '${t}', new.id_global, '${e}', bio_hlc(), new.${e}\n     WHERE new.${e} IS NOT old.${e}\n  ${v};`).join(`
`);return[`CREATE TRIGGER IF NOT EXISTS ${_(t,`ins`)} AFTER INSERT ON ${t} BEGIN
  INSERT INTO ${g} (tabela, id_global, campo, hlc, valor) VALUES
${r}
  ${v};
END;`,`CREATE TRIGGER IF NOT EXISTS ${_(t,`upd`)} AFTER UPDATE ON ${t} BEGIN
${i}
END;`,`CREATE TRIGGER IF NOT EXISTS ${_(t,`del`)} AFTER DELETE ON ${t} BEGIN
  ...
  ${v};
END;`]}function oe(e){return[`ins`,`upd`,`del`].map(t=>`DROP TRIGGER IF EXISTS ${_(e,t)}`)}function se(e){let t=h(e).filter(e=>e.tipo===`tabela`&&e.campos.includes(`id_global`)).flatMap(ae);return[`CREATE TABLE IF NOT EXISTS bioma_carimbos (
...
) STRICT, WITHOUT ROWID;`,re,...t]}const ce=`SELECT bio_hlc_visto(hlc) FROM ${g}
  ORDER BY hlc DESC LIMIT 1`;function y(e){return`"${e.replace(/"/g,`""`)}"`}function le(e){return e.map(e=>({nome:String(e.name),tipo:String(e.type).toUpperCase(),notNulo:Number(e.notnull)===1,chave:Number(e.pk)>0,temPadrao:e.dflt_value!==null&&e.dflt_value!==void 0}))}function b(e,t){if(t==null)return null;if(e.tipo===`TEXT`)return typeof t==`string`?t:String(t);if(e.tipo===`BLOB`)return t instanceof Uint8Array?t:null;if(typeof t==`boolean`)return+!!t;let n=typeof t==`number`?t:Number(t);return Number.isFinite(n)?n:null}function x(e,t,n){let r=t.filter(e=>n[e.nome]!==void 0);if(r.length===0)return{sql:`INSERT INTO ${y(e)} DEFAULT VALUES`,parametros:[]};let i=r.map(e=>y(e.nome)).join(`, `),a=r.map(()=>`?`).join(`, `);return{sql:`INSERT INTO ${y(e)} (${i}) VALUES (${a})`,parametros:r.map(e=>b(e,n[e.nome]))}}function S(e,t,n){let r=t.filter(e=>n[e.nome]!==void 0),i=t.filter(e=>e.chave).map(e=>e.nome),a=r.filter(e=>!i.includes(e.nome)&&e.nome!==`id_global`),o=r.map(e=>y(e.nome)).join(`, `),s=r.map(()=>`?`).join(`, `),c=i.map(y).join(`, `),l=a.length===0?`DO NOTHING`:`DO UPDATE SET ${a.map(e=>`${y(e.nome)} = excluded.${y(e.nome)}`).join(`, `)}`;return{sql:`INSERT INTO ${y(e)} (${o}) VALUES (${s}) ON CONFLICT(${c}) ${l}`,parametros:r.map(e=>b(e,n[e.nome]))}}function ue(e){return e===null||e.byteLength===0?`novo`:new TextDecoder(`latin1`).decode(e.subarray(0,16))===`SQLite format 3\0`?`em-claro`:`cifrado`}const de=`chave-do-aparelho`;function fe(e){return e instanceof Uint8Array||e instanceof ArrayBuffer?new Uint8Array(e):null}async function pe(e,t){let n=fe(await e(de));if(n!==null){if(n.byteLength!==32)throw Error(`a chave guardada neste aparelho tem ${n.byteLength} bytes, e não 32: o banco cifrado não abre sem ela. Apague os dados deste aparelho para recomeçar.`);return n}let r=crypto.getRandomValues(/* @__PURE__ */ new Uint8Array(32));return await t(de,r),r}function me(e,t){return e.consultar(`SELECT name FROM pragma_table_info(?)`,[t]).map(e=>String(e.name))}function he(e,t,n){let r=me(e,t);if(r.length===0)return!1;let i=n.filter(([e])=>!r.includes(e));if(i.length===0)return!1;for(let[n,r]of i)e.executar(`ALTER TABLE ${t} ADD COLUMN ${n} ${r}`);for(let n of oe(t))e.executar(n);return!0}var ge=class{bd;catalogo;colunas=/* @__PURE__ */ new Map;avisadas=/* @__PURE__ */ new Set;profundidade=0;constructor(e,t){this.bd=e,this.catalogo=t}emTransacao(e){return this.naTransacao(e)}naTransacao(e){if(this.profundidade>0)return e();this.profundidade+=1;try{return this.bd.emTransacao(e)}finally{--this.profundidade}}stores(){return Object.keys(this.catalogo)}storesDa(e){return this.stores().filter(t=>this.catalogo[t]?.familia===e)}storeDe(e){let t=this.catalogo[e];if(t===void 0)throw Error(`store desconhecido: ${e}`);return t}lerEsquemaAplicado(){this.colunas.clear();for(let e of this.stores()){let t=this.storeDe(e);for(let e of[t.tabela,...(t.filhos??[]).map(e=>e.tabela)])this.colunas.has(e)||this.colunas.set(e,le(this.bd.consultar(`PRAGMA table_info(${y(e)})`)))}}tabelas(){return[...this.colunas.keys()]}colunasDa(e){let t=this.colunas.get(e);if(t===void 0||t.length===0)throw Error(`tabela ausente no banco: ${e}`);return t}todos(e){let t=this.storeDe(e);return this.comFilhos(t,this.bd.consultar(`SELECT * FROM ${y(t.tabela)} ORDER BY id`))}obter(e,t){let n=this.storeDe(e),r=this.bd.consultar(`SELECT * FROM ${y(n.tabela)} WHERE id = ?`,[Number(t)]);return this.comFilhos(n,r)[0]}contar(e){return Number(this.bd.escalar(`SELECT COUNT(*) FROM ${y(this.storeDe(e).tabela)}`)??0)}salvar(e,t){let n=this.storeDe(e),r=C(n,t),i=w(n,T(n,r));this.avisarCamposIgnorados(n.tabela,i);let a=0;return this.naTransacao(()=>{let e=r.id;if(e==null){let e=x(n.tabela,this.colunasDa(n.tabela),i);this.bd.executar(e.sql,e.parametros),a=Number(this.bd.escalar(`SELECT MAX(id) FROM ${y(n.tabela)}`)??0)}else{a=Number(e);let t=S(n.tabela,this.colunasDa(n.tabela),{...i,id:a});this.bd.executar(t.sql,t.parametros)}this.trocarFilhos(n,a,r)}),a}excluir(e,t){let n=this.storeDe(e);this.bd.executar(`DELETE FROM ${y(n.tabela)} WHERE id = ?`,[Number(t)])}substituirTudo(e,t){let n=this.storeDe(e);this.naTransacao(()=>{this.bd.executar(`DELETE FROM ${y(n.tabela)}`);let e=this.colunasDa(n.tabela),r=0;for(let i of t){let t=Number(i.id);Number.isFinite(t)?t>r&&(r=t):t=++r;let a=C(n,{...i,id:t}),o=w(n,T(n,a));this.avisarCamposIgnorados(n.tabela,o);let s=x(n.tabela,e,o);this.bd.executar(s.sql,s.parametros),this.trocarFilhos(n,t,a)}})}comFilhos(e,t){let n=e.filhos??[],r=t.map(t=>_e(e,t));if(n.length===0||r.length===0)return r;let i=r.map(e=>Number(e.id)),a=i.map(()=>`?`).join(`, `),o=r;for(let e of n){let t=this.bd.consultar(`SELECT * FROM ${y(e.tabela)} WHERE ${y(e.chave)} IN (${a}) ORDER BY id`,i),n=/* @__PURE__ */ new Map;for(let r of t){let t=Number(r[e.chave]),i=n.get(t);i===void 0?n.set(t,[r]):i.push(r)}o=o.map(t=>({...t,[e.campo]:n.get(Number(t.id))??[]}))}return o}trocarFilhos(e,t,n){for(let r of e.filhos??[]){let e=n[r.campo];if(!Array.isArray(e))continue;this.bd.executar(`DELETE FROM ${y(r.tabela)} WHERE ${y(r.chave)} = ?`,[t]);let i=this.colunasDa(r.tabela),a=e[0];a!==void 0&&this.avisarCamposIgnorados(r.tabela,{...a,[r.chave]:t});let o=0;for(let n of e){let e=Number(n.id);Number.isFinite(e)?e>o&&(o=e):e=++o;let a=x(r.tabela,i,{...n,id:e,[r.chave]:t});this.bd.executar(a.sql,a.parametros)}}}avisarCamposIgnorados(e,t){if(this.avisadas.has(e))return;let n=new Set(this.colunasDa(e).map(e=>e.nome)),r=Object.keys(t).filter(e=>!n.has(e));r.length!==0&&(this.avisadas.add(e),console.warn(`${e}: campo(s) que a tabela não tem, descartado(s) na gravação: ${r.join(`, `)}.`))}};function C(e,t){let n=e.apelidos;if(n===void 0)return t;let r={...t},i=!1;for(let[e,t]of Object.entries(n))e in r&&(r[t]===void 0&&(r[t]=r[e]),delete r[e],i=!0);return i?r:t}function w(e,t){let n=e.json;if(n===void 0)return t;let r={...t};for(let e of n){let t=r[e];t!=null&&typeof t!=`string`&&(r[e]=JSON.stringify(t))}return r}function _e(e,t){let n=e.json;if(n===void 0)return t;let r={...t};for(let e of n){let t=r[e];if(typeof t==`string`)try{r[e]=JSON.parse(t===``?`null`:t)}catch{r[e]=t.trimStart().startsWith(`{`)?{}:[]}}return r}function T(e,t){let n=e.filhos??[];if(n.length===0)return t;let r={...t};for(let e of n)delete r[e.campo];return r}var ve=class{bytes;usados=0;constructor(){this.bytes=/* @__PURE__ */ new Uint8Array(65536)}ler(e,t){if(t>=this.usados)return 0;let n=Math.min(e.length,this.usados-t);return e.set(this.bytes.subarray(t,t+n)),n}escrever(e,t){return this.reservar(t+e.length),this.bytes.set(e,t),this.usados=Math.max(this.usados,t+e.length),e.length}tamanho(){return this.usados}truncar(e){this.reservar(e),e<this.usados&&this.bytes.fill(0,e,this.usados),this.usados=e}sincronizar(){}fechar(){}reservar(e){if(e<=this.bytes.length)return;let t=this.bytes.length;for(;t<e;)t*=2;let n=new Uint8Array(t);n.set(this.bytes.subarray(0,this.usados)),this.bytes=n}},ye=class{arquivos=/* @__PURE__ */ new Map;pastas=/* @__PURE__ */ new Set([`/`]);abrir(e,t){let n=this.arquivos.get(e);if(n!==void 0)return t.exclusivo?null:(t.truncar&&n.truncar(0),n);if(!t.criar)return null;let r=new ve;return this.arquivos.set(e,r),r}existe(e){return this.arquivos.has(e)||this.pastas.has(e)}ehPasta(e){return this.pastas.has(e)}tamanhoDe(e){return this.arquivos.get(e)?.tamanho()??null}apagar(e){return this.arquivos.delete(e)}criarPasta(e){return!this.pastas.has(e)&&(this.pastas.add(e),!0)}apagarPasta(e){return this.pastas.delete(e)}};const E={OK:0,BADF:8,EXIST:20,INVAL:28,ISDIR:31,NOENT:44,NOSYS:52,NOTDIR:54,NOTEMPTY:55},D={DIRETORIO:3,ARQUIVO:4,CARACTERE:2},O={CRIAR:1,PASTA:2,EXCLUSIVO:4,TRUNCAR:8},k={INICIO:0,ATUAL:1,FIM:2},A={FDSTAT:24,FILESTAT:64,PRESTAT:8};var be=class{arquivos;memoria=null;descritores=/* @__PURE__ */ new Map;proximo=4;constructor(e){this.arquivos=e}ligar(e){this.memoria=e}get vista(){if(this.memoria===null)throw Error("hospedeiro WASI usado antes de `ligar()`");return new DataView(this.memoria.buffer)}get bytes(){if(this.memoria===null)throw Error("hospedeiro WASI usado antes de `ligar()`");return new Uint8Array(this.memoria.buffer)}caminhoEm(e,t){let n=new TextDecoder().decode(this.bytes.subarray(e,e+t));return n.startsWith(`/`)?n:`/${n}`}get importacoes(){return{fd_close:this.fd_close,fd_read:this.fd_read,fd_write:this.fd_write,fd_seek:this.fd_seek,fd_sync:this.fd_sync,fd_fdstat_get:this.fd_fdstat_get,fd_fdstat_set_flags:()=>E.OK,fd_filestat_get:this.fd_filestat_get,fd_filestat_set_size:this.fd_filestat_set_size,fd_prestat_get:this.fd_prestat_get,fd_prestat_dir_name:this.fd_prestat_dir_name,path_open:this.path_open,path_create_directory:this.path_create_directory,path_remove_directory:this.path_remove_directory,path_filestat_get:this.path_filestat_get,path_filestat_set_times:()=>E.OK,path_unlink_file:this.path_unlink_file,path_readlink:()=>E.INVAL,environ_get:()=>E.OK,environ_sizes_get:this.environ_sizes_get,clock_time_get:this.clock_time_get,random_get:this.random_get,poll_oneoff:()=>E.NOSYS,proc_exit:e=>{throw Error(`o motor chamou proc_exit(${e}) — isto é defeito, não fluxo`)}}}fd_close=e=>{let t=this.descritores.get(e);return t===void 0?E.BADF:(t.arquivo.fechar(),this.descritores.delete(e),E.OK)};fd_read=(e,t,n,r)=>{let i=this.descritores.get(e);if(i===void 0)return E.BADF;let a=0;for(let e=0;e<n;e++){let n=this.vista.getUint32(t+e*8,!0),r=this.vista.getUint32(t+e*8+4,!0);if(r===0)continue;let o=new Uint8Array(r),s=i.arquivo.ler(o,i.posicao);if(this.bytes.set(o.subarray(0,s),n),i.posicao+=s,a+=s,s<r)break}return this.vista.setUint32(r,a,!0),E.OK};fd_write=(e,t,n,r)=>{let i=0;for(let r=0;r<n;r++){let n=this.vista.getUint32(t+r*8,!0),a=this.vista.getUint32(t+r*8+4,!0),o=this.bytes.subarray(n,n+a);if(e===1||e===2){let t=new TextDecoder().decode(o).trimEnd();t!==``&&console[e===2?`error`:`log`](`[Bioma SQL] ${t}`),i+=a;continue}let s=this.descritores.get(e);if(s===void 0)return E.BADF;let c=s.arquivo.escrever(o.slice(),s.posicao);s.posicao+=c,i+=c}return this.vista.setUint32(r,i,!0),E.OK};fd_seek=(e,t,n,r)=>{let i=this.descritores.get(e);if(i===void 0)return E.BADF;let a=(n===k.INICIO?0:n===k.ATUAL?i.posicao:i.arquivo.tamanho())+Number(t);return a<0?E.INVAL:(i.posicao=a,this.vista.setBigUint64(r,BigInt(a),!0),E.OK)};fd_sync=e=>{let t=this.descritores.get(e);return t===void 0?E.BADF:(t.arquivo.sincronizar(),E.OK)};fd_fdstat_get=(e,t)=>{let n=e===3?D.DIRETORIO:e<=2?D.CARACTERE:this.descritores.has(e)?D.ARQUIVO:null;return n===null?E.BADF:(this.bytes.fill(0,t,t+A.FDSTAT),this.vista.setUint8(t,n),this.vista.setBigUint64(t+8,18446744073709551615n,!0),this.vista.setBigUint64(t+16,18446744073709551615n,!0),E.OK)};fd_filestat_get=(e,t)=>{let n=this.descritores.get(e);return n===void 0?e===3?this.escreverStat(t,D.DIRETORIO,0):E.BADF:this.escreverStat(t,D.ARQUIVO,n.arquivo.tamanho())};fd_filestat_set_size=(e,t)=>{let n=this.descritores.get(e);return n===void 0?E.BADF:(n.arquivo.truncar(Number(t)),E.OK)};fd_prestat_get=(e,t)=>e===3?(this.vista.setUint8(t,0),this.vista.setUint32(t+4,1,!0),E.OK):E.BADF;fd_prestat_dir_name=(e,t,n)=>e===3?(this.bytes.set(new TextEncoder().encode(`/`).subarray(0,n),t),E.OK):E.BADF;path_open=(e,t,n,r,i,a,o,s,c)=>{let l=this.caminhoEm(n,r);if((i&O.PASTA)!==0)return this.arquivos.ehPasta(l)?E.OK:E.NOTDIR;if(this.arquivos.ehPasta(l))return E.ISDIR;let u=(i&O.CRIAR)!==0,d=(i&O.EXCLUSIVO)!==0,f=this.arquivos.existe(l);if(d&&f)return E.EXIST;if(!u&&!f)return E.NOENT;let p=this.arquivos.abrir(l,{criar:u,truncar:(i&O.TRUNCAR)!==0,exclusivo:d});if(p===null)return f?E.EXIST:E.NOENT;let m=this.proximo++;return this.descritores.set(m,{arquivo:p,caminho:l,posicao:0}),this.vista.setUint32(c,m,!0),E.OK};path_create_directory=(e,t,n)=>this.arquivos.criarPasta(this.caminhoEm(t,n))?E.OK:E.EXIST;path_remove_directory=(e,t,n)=>this.arquivos.apagarPasta(this.caminhoEm(t,n))?E.OK:E.NOENT;path_filestat_get=(e,t,n,r,i)=>{let a=this.caminhoEm(n,r);if(this.arquivos.ehPasta(a))return this.escreverStat(i,D.DIRETORIO,0);let o=this.arquivos.tamanhoDe(a);return o===null?E.NOENT:this.escreverStat(i,D.ARQUIVO,o)};path_unlink_file=(e,t,n)=>this.arquivos.apagar(this.caminhoEm(t,n))?E.OK:E.NOENT;environ_sizes_get=(e,t)=>(this.vista.setUint32(e,0,!0),this.vista.setUint32(t,0,!0),E.OK);clock_time_get=(e,t,n)=>(this.vista.setBigUint64(n,BigInt(Date.now())*1000000n,!0),E.OK);random_get=(e,t)=>{let n=this.bytes.subarray(e,e+t);for(let e=0;e<t;e+=65536)crypto.getRandomValues(n.subarray(e,Math.min(e+65536,t)));return E.OK};escreverStat(e,t,n){return this.bytes.fill(0,e,e+A.FILESTAT),this.vista.setUint8(e+16,t),this.vista.setBigUint64(e+24,1n,!0),this.vista.setBigUint64(e+32,BigInt(n),!0),E.OK}};const j=new TextEncoder,M={OK:0,NAO_E_BANCO:26,LINHA:100,FIM:101},N={INTEIRO:1,REAL:2,TEXTO:3,BLOB:4,NULO:5},xe={9:`DELETE`,18:`INSERT`,23:`UPDATE`};var Se=class{atendentes=null;ligar(e){this.atendentes=e}importacoes={rastro:(e,t,n)=>{this.atendentes?.rastro(t,n)},mudanca:(e,t,n,r,i,a)=>{this.atendentes?.mudanca(t,n,r,i,a)},progresso:e=>this.atendentes?.progresso()??0}};async function Ce(e){let t=new be(e.arquivos??new ye),n=new Se,r=(await we(e.modulo,t,n)).exports;return t.ligar(r.memory),r._initialize(),new Te(r,n,e.caminho??`/bioma.sqlite`,e.chave)}async function we(e,t,n){let r={wasi_snapshot_preview1:t.importacoes,bioma:n.importacoes};if(e instanceof WebAssembly.Module)return WebAssembly.instantiate(e,r);if(e instanceof Response||e instanceof Promise)try{let{instance:t}=await WebAssembly.instantiateStreaming(e,r);return t}catch{let t=await e,{instance:n}=await WebAssembly.instantiate(await t.clone().arrayBuffer(),r);return n}return WebAssembly.instantiate(await WebAssembly.compile(e),r)}var Te=class{motor;indice;celula;fechado=!1;ouvinteDoRastro=null;ouvinteDaMudanca=null;decidir=null;falhaNoGancho=null;interrompido=!1;constructor(e,t,n,r){if(this.motor=e,this.celula=e.biomasql_wasm_alocar(4),this.celula===0)throw Error(`Bioma SQL: o motor não alocou nem quatro bytes`);let i=r===void 0?/* @__PURE__ */ new Uint8Array:typeof r==`string`?j.encode(r):r,a=this.escreverTexto(n),o=i.byteLength===0?0:this.escreverBytes(i);try{let t=e.biomasql_wasm_abrir(a,1,o,i.byteLength);if(t<0)throw Error(-t===M.NAO_E_BANCO?`Bioma SQL: ${n} não abriu — a chave não confere, ou o arquivo não é um banco`:`Bioma SQL não abriu ${n} (código ${-t})`);this.indice=t}finally{e.biomasql_wasm_liberar(a),o!==0&&(new Uint8Array(e.memory.buffer).fill(0,o,o+i.byteLength),e.biomasql_wasm_liberar(o))}t.ligar({rastro:(e,t)=>this.aoRastro(e,t),mudanca:(e,t,n,r,i)=>this.aoMudar(e,t,n,r,i),progresso:()=>this.aoProgredir()})}get versao(){return this.lerTexto(this.motor.biomasql_wasm_versao())}executar(e,t){let n=this.preparar(e,t);try{let t=this.motor.biomasql_wasm_passo(n);return t!==M.FIM&&t!==M.LINHA&&this.lancar(e),this.motor.biomasql_wasm_alteracoes(this.indice)}finally{this.motor.biomasql_wasm_finalizar(n),this.conferirGanchos()}}executarRoteiro(e){if(this.fechado)throw Error(`Bioma SQL: o banco já foi fechado`);let t=this.escreverTexto(e),n;try{n=this.motor.biomasql_wasm_roteiro(this.indice,t)}finally{this.motor.biomasql_wasm_liberar(t)}n!==M.OK&&this.lancar(e),this.conferirGanchos()}consultar(e,t){let n=this.preparar(e,t);try{let t=this.motor.biomasql_wasm_colunas(n),r=[];for(let e=0;e<t;e++)r.push(this.lerTexto(this.motor.biomasql_wasm_nome_coluna(n,e)));let i=[],a=this.motor.biomasql_wasm_passo(n);for(;a===M.LINHA;){let e={};for(let i=0;i<t;i++)e[r[i]]=this.lerColuna(n,i);i.push(e),a=this.motor.biomasql_wasm_passo(n)}return a!==M.FIM&&this.lancar(e),i}finally{this.motor.biomasql_wasm_finalizar(n),this.conferirGanchos()}}escalar(e,t){let n=this.preparar(e,t);try{let t=this.motor.biomasql_wasm_passo(n);return t!==M.LINHA&&t!==M.FIM&&this.lancar(e),t===M.LINHA?this.lerColuna(n,0):null}finally{this.motor.biomasql_wasm_finalizar(n),this.conferirGanchos()}}emTransacao(e){this.executar(`BEGIN`);try{let t=e();return this.executar(`COMMIT`),t}catch(e){throw this.executar(`ROLLBACK`),e}}integridadeIncremental(e=`main`){return this.consultar(`SELECT objeto, mensagem FROM bio_intck(?)`,[e]).map(e=>({objeto:e.objeto===null?null:String(e.objeto),mensagem:String(e.mensagem)}))}recuperar(e={}){return this.consultar(`SELECT sql FROM bio_recover(?, ?)`,[e.esquema??`main`,e.perdidos??null]).map(e=>String(e.sql))}recomendarIndices(e,t){return(t===void 0?this.consultar(`SELECT * FROM bio_expert(?)`,[e]):this.consultar(`SELECT * FROM bio_expert(?, ?)`,[e,t])).map(e=>({consulta:String(e.consulta??``),indices:e.indices===null?null:String(e.indices).trim(),plano:e.plano===null?null:String(e.plano).trim(),candidatos:e.candidatos===null?null:String(e.candidatos).trim()}))}rastrear(e){if(this.fechado)throw Error(`Bioma SQL: o banco já foi fechado`);this.ouvinteDoRastro=e,this.motor.biomasql_wasm_rastrear(this.indice,e===null?0:1)}observar(e){if(this.fechado)throw Error(`Bioma SQL: o banco já foi fechado`);this.ouvinteDaMudanca=e,this.motor.biomasql_wasm_observar(this.indice,e===null?0:1)}vigiar(e,t){if(this.fechado)throw Error(`Bioma SQL: o banco já foi fechado`);let n=t!==null&&e>0;this.decidir=n?t:null,this.motor.biomasql_wasm_vigiar(this.indice,n?e:0)}interromper(){if(this.fechado)throw Error(`Bioma SQL: o banco já foi fechado`);this.motor.biomasql_wasm_interromper(this.indice)}serializar(){if(this.fechado)throw Error(`Bioma SQL: o banco já foi fechado`);let e=this.motor.biomasql_wasm_serializar(this.indice,this.celula);if(e===0)throw Error(`Bioma SQL: o motor não serializou o banco`);try{return new Uint8Array(this.motor.memory.buffer,e,this.lerCelula()).slice()}finally{this.motor.biomasql_wasm_liberar(e)}}desserializar(e){if(this.fechado)throw Error(`Bioma SQL: o banco já foi fechado`);let t=this.motor.biomasql_wasm_alocar(e.byteLength);if(t===0)throw Error(`Bioma SQL: não coube o banco de ${e.byteLength} bytes na memória`);new Uint8Array(this.motor.memory.buffer).set(e,t);let n=this.motor.biomasql_wasm_desserializar(this.indice,t,e.byteLength);if(n!==M.OK)throw Error(`Bioma SQL: o conteúdo não foi aceito como banco (código ${n})`)}rechavear(e){if(this.fechado)throw Error(`Bioma SQL: o banco já foi fechado`);let t=e===void 0?/* @__PURE__ */ new Uint8Array:typeof e==`string`?j.encode(e):e,n=t.byteLength===0?0:this.escreverBytes(t);try{let e=this.motor.biomasql_wasm_rechavear(this.indice,n,t.byteLength);if(e!==M.OK)throw Error(`Bioma SQL: a troca de chave falhou (código ${e})`)}finally{n!==0&&this.motor.biomasql_wasm_liberar(n)}}fechar(){this.fechado||(this.fechado=!0,this.motor.biomasql_wasm_fechar(this.indice),this.motor.biomasql_wasm_liberar(this.celula))}preparar(e,t){if(this.fechado)throw Error(`Bioma SQL: o banco já foi fechado`);let n=this.escreverTexto(e),r;try{r=this.motor.biomasql_wasm_preparar(this.indice,n,this.celula)}finally{this.motor.biomasql_wasm_liberar(n)}if(r<0&&this.lancar(e),this.lerCelula()!==0)throw this.motor.biomasql_wasm_finalizar(r),Error(`Bioma SQL: o SQL traz mais de um comando; use executarRoteiro\n  SQL: ${e}`);try{this.ligar(r,t)}catch(e){throw this.motor.biomasql_wasm_finalizar(r),e}return r}ligar(e,t){if(t!==void 0){if(Array.isArray(t)){for(let n=0;n<t.length;n++)this.ligarEm(e,n+1,t[n]);return}for(let[n,r]of Object.entries(t)){let t=/^[:@$]/.test(n)?n:`:${n}`,i=this.escreverTexto(t),a;try{a=this.motor.biomasql_wasm_indice_parametro(e,i)}finally{this.motor.biomasql_wasm_liberar(i)}if(a===0)throw Error(`Bioma SQL: o SQL não declara o parâmetro ${t}`);this.ligarEm(e,a,r)}}}ligarEm(e,t,n){if(n===null){this.motor.biomasql_wasm_ligar_nulo(e,t);return}if(typeof n==`bigint`){this.motor.biomasql_wasm_ligar_inteiro(e,t,n);return}if(typeof n==`number`){Number.isInteger(n)?this.motor.biomasql_wasm_ligar_inteiro(e,t,BigInt(n)):this.motor.biomasql_wasm_ligar_real(e,t,n);return}if(n instanceof Uint8Array){let r=this.escreverBytes(n);try{this.motor.biomasql_wasm_ligar_blob(e,t,r,n.byteLength)}finally{this.motor.biomasql_wasm_liberar(r)}return}let r=new TextEncoder().encode(n),i=this.motor.biomasql_wasm_alocar(r.length+1);if(i===0)throw Error(`Bioma SQL: o motor não alocou o valor a ligar`);try{new Uint8Array(this.motor.memory.buffer).set(r,i),this.motor.biomasql_wasm_ligar_texto(e,t,i,r.length)}finally{this.motor.biomasql_wasm_liberar(i)}}lerColuna(e,t){switch(this.motor.biomasql_wasm_tipo_coluna(e,t)){case N.NULO:return null;case N.INTEIRO:{let n=this.motor.biomasql_wasm_inteiro(e,t);return n>=-9007199254740991n&&n<=9007199254740991n?Number(n):n}case N.REAL:return this.motor.biomasql_wasm_real(e,t);case N.BLOB:{let n=this.motor.biomasql_wasm_blob(e,t),r=this.motor.biomasql_wasm_bytes(e,t);return n===0||r===0?/* @__PURE__ */ new Uint8Array:new Uint8Array(this.motor.memory.buffer,n,r).slice()}default:{let n=this.motor.biomasql_wasm_texto(e,t),r=this.motor.biomasql_wasm_bytes(e,t);return new TextDecoder().decode(new Uint8Array(this.motor.memory.buffer,n,r).slice())}}}aoRastro(e,t){let n=this.ouvinteDoRastro;if(n!==null)try{n({sql:this.lerTexto(e),ms:Number(t)/1e6})}catch(e){this.guardarFalha(e)}}aoMudar(e,t,n,r,i){let a=this.ouvinteDaMudanca;if(a!==null)try{let o=xe[e];if(o===void 0)return;a({operacao:o,esquema:this.lerTexto(t),tabela:this.lerTexto(n),chaveAntiga:o===`INSERT`?null:r,chaveNova:o===`DELETE`?null:i,antiga:o===`INSERT`?null:this.linhaPre(1),nova:o===`DELETE`?null:this.linhaPre(0)})}catch(e){this.guardarFalha(e)}}aoProgredir(){let e=this.decidir;if(e===null)return 0;try{return e()?0:(this.interrompido=!0,1)}catch(e){return this.guardarFalha(e),1}}linhaPre(e){let t=this.motor.biomasql_wasm_pre_contagem(this.indice),n=[];for(let r=0;r<t;r++)n.push(this.lerValorPre(r,e));return n}lerValorPre(e,t){switch(this.motor.biomasql_wasm_pre_tipo(this.indice,e,t)){case N.NULO:return null;case N.INTEIRO:{let n=this.motor.biomasql_wasm_pre_inteiro(this.indice,e,t);return n>=-9007199254740991n&&n<=9007199254740991n?Number(n):n}case N.REAL:return this.motor.biomasql_wasm_pre_real(this.indice,e,t);case N.BLOB:{let n=this.motor.biomasql_wasm_pre_blob(this.indice,e,t),r=this.motor.biomasql_wasm_pre_bytes(this.indice,e,t);return n===0||r===0?/* @__PURE__ */ new Uint8Array:new Uint8Array(this.motor.memory.buffer,n,r).slice()}default:{let n=this.motor.biomasql_wasm_pre_texto(this.indice,e,t),r=this.motor.biomasql_wasm_pre_bytes(this.indice,e,t);return n===0?``:new TextDecoder().decode(new Uint8Array(this.motor.memory.buffer,n,r).slice())}}}guardarFalha(e){this.falhaNoGancho??=e instanceof Error?e:Error(String(e))}conferirGanchos(){let e=this.falhaNoGancho;if(this.falhaNoGancho=null,e!==null)throw e}escreverBytes(e){let t=this.motor.biomasql_wasm_alocar(e.byteLength+1);if(t===0)throw Error(`Bioma SQL: o motor não alocou o bloco pedido`);return new Uint8Array(this.motor.memory.buffer).set(e,t),t}escreverTexto(e){let t=new TextEncoder().encode(e),n=this.motor.biomasql_wasm_alocar(t.length+1),r=new Uint8Array(this.motor.memory.buffer);return r.set(t,n),r[n+t.length]=0,n}lerCelula(){return new Int32Array(this.motor.memory.buffer,this.celula,1)[0]}lerTexto(e){if(e===0)return``;let t=new Uint8Array(this.motor.memory.buffer),n=e;for(;t[n]!==0;)n++;return new TextDecoder().decode(t.subarray(e,n))}lancar(e){if(this.conferirGanchos(),this.interrompido)throw this.interrompido=!1,Error(`Bioma SQL: consulta interrompida pelo vigia\n  SQL: ${e}`);let t=this.lerTexto(this.motor.biomasql_wasm_erro(this.indice));throw Error(`Bioma SQL: ${t}\n  SQL: ${e}`)}},Ee=new URL(`biomasql-iGKX8QLL.wasm`,import.meta.url).href,De=class{vaga;constructor(e){this.vaga=e}ler(e,t){return this.vaga.punho.read(e,{at:t})}escrever(e,t){return this.vaga.punho.write(e,{at:t})}tamanho(){return this.vaga.punho.getSize()}truncar(e){this.vaga.punho.truncate(e)}sincronizar(){this.vaga.punho.flush()}fechar(){}},Oe=class e{vagas;caminhoDoBanco;pastas=/* @__PURE__ */ new Set([`/`,`/.`]);constructor(e,t){this.vagas=e,this.caminhoDoBanco=t}static async preparar(t){let n=await(await navigator.storage.getDirectory()).getDirectoryHandle(t.pasta,{create:!0}),r=/* @__PURE__ */ new Map;for(let e of[``,`-journal`,`-wal`]){let i=`${t.banco}${e}`,a=await(await n.getFileHandle(i,{create:!0})).createSyncAccessHandle();r.set(`/${i}`,{punho:a,existe:e===``&&a.getSize()>0})}return new e(r,`/${t.banco}`)}abrir(e,t){let n=this.vagas.get(e);return n===void 0||t.exclusivo&&n.existe||!n.existe&&!t.criar?null:(t.truncar&&n.punho.truncate(0),n.existe=!0,new De(n))}existe(e){return this.vagas.get(e)?.existe??this.pastas.has(e)}ehPasta(e){return this.pastas.has(e)}tamanhoDe(e){let t=this.vagas.get(e);return t?.existe?t.punho.getSize():null}apagar(e){let t=this.vagas.get(e);return t===void 0||!t.existe?!1:(t.punho.truncate(0),t.punho.flush(),t.existe=!1,!0)}criarPasta(e){return!this.pastas.has(e)&&(this.pastas.add(e),!0)}apagarPasta(e){return this.pastas.delete(e)}encerrar(){for(let e of this.vagas.values())e.punho.close();this.vagas.clear()}};const P=[{tabela:`not_perguntas`,colunas:[`pergunta`,`referencia`,`resposta1`,`resposta2`,`resposta3`,`resposta4`,`explicacao`]},{tabela:`not_poesias`,colunas:[`titulo`,`conteudo`]},{tabela:`not_receitas`,colunas:[`titulo`,`categoria`,`ingredientes`,`instrucoes`]},{tabela:`not_cronologia_eventos`,colunas:[`titulo`,`resumo`,`referencia`,`periodo`,`obra_fonte`]},{tabela:`not_principios`,colunas:[`titulo`,`principio`,`referencia`,`explicacao`,`pratica`,`reflexoes`]},{tabela:`not_imite_cartoes`,colunas:[`titulo`,`tema`,`personagens`,`cenario`,`julgamento`,`correta`,`reenquadramento`,`referencia`,`espelho`]},{tabela:`not_criacao_modulos`,colunas:[`titulo`,`categoria`,`conceito`,`desafio`,`reflexao`,`referencia`]}];function F(e){return`${e}_fts`}function I({tabela:e,colunas:t,chave:n=`id`},{vocabulario:r=!1,indiceDePublicar:i=!1}={}){let a=F(e),o=t.join(`, `),s=t.map(e=>`new.${e}`).join(`, `),c=t.map(e=>`old.${e}`).join(`, `);return[`CREATE VIRTUAL TABLE IF NOT EXISTS ${a} USING fts5(
      ${o},
      content = '${e}',
      content_rowid = '${n}',
      tokenize = "unicode61 remove_diacritics 2"
    );`,`CREATE TRIGGER IF NOT EXISTS ${a}_ai AFTER INSERT ON ${e} BEGIN
      INSERT INTO ${a} (rowid, ${o}) VALUES (new.${n}, ${s});
    END;`,`CREATE TRIGGER IF NOT EXISTS ${a}_ad AFTER DELETE ON ${e} BEGIN
      INSERT INTO ${a} (${a}, rowid, ${o}) VALUES ('delete', old.${n}, ${c});
    END;`,`CREATE TRIGGER IF NOT EXISTS ${a}_au AFTER UPDATE ON ${e} BEGIN
      INSERT INTO ${a} (${a}, rowid, ${o}) VALUES ('delete', old.${n}, ${c});
      INSERT INTO ${a} (rowid, ${o}) VALUES (new.${n}, ${s});
    END;`,...r?[`CREATE VIRTUAL TABLE IF NOT EXISTS ${e}_vocab USING fts5vocab(${a}, row);`]:[],...i?[`CREATE INDEX IF NOT EXISTS idx_${e}_publicar ON ${e} (id) WHERE publicar = 1;`]:[]]}function L(e){return e.map(({tabela:e})=>{let t=F(e);return`INSERT INTO ${t} (${t}) VALUES ('rebuild');`})}P.map(({tabela:e})=>`SELECT '${e}' AS acervo, term, doc, cnt FROM ${e}_vocab`).join(` UNION ALL `),P.flatMap(e=>I(e,{vocabulario:!0,indiceDePublicar:!0})),L(P),Math.max(...[{versao:1,nome:`esquemaBase`,oQue:`o esquema canônico inteiro, com as sementes, o Tutorial e o catálogo de fábrica`},{versao:2,nome:`ordemNoAno`,oQue:"a coluna `ordem_no_ano` da Cronologia, que o `ordem_absoluta` não sabe dar"},{versao:3,nome:`cartoesImite`,oQue:`a tabela e o acervo de "Imite a Sua Fé"`},{versao:4,nome:`principiosBiblicos`,oQue:`a tabela e o acervo dos Princípios Bíblicos`},{versao:5,nome:`aplicarBusca`,oQue:"a FTS5 dos sete acervos, os gatilhos e o `rebuild`"},{versao:6,nome:`umaFonteDeVersao`,oQue:"a `_migracoes` do migrador legado sai: `bioma_esquema` fica como única"},{versao:7,nome:`limiteEmCentavos`,oQue:"o `limite_mensal` da categoria vira centavo em `INTEGER`, como o dinheiro do Kobi Note"},{versao:8,nome:`bancada`,oQue:`a bancada: toda tabela do Kobi Note e do Kobi Flow passa a existir também no admin`},{versao:9,nome:`perguntasFrequentes`,oQue:`a tabela de "Respostas Frequentes" e o tópico do Tutorial`},{versao:10,nome:`perguntasComLentes`,oQue:`"Respostas Frequentes" vira exercício de lentes: a tabela é refeita e o acervo velho sai`},{versao:11,nome:`filaDaSincronizacao`,oQue:`a fila e o diário de conflitos do Kobi Flow ganham as colunas que o motor da sincronização lê`},{versao:12,nome:`jogoNaBancada`,oQue:`o progresso e as respostas do Você se lembra? chegam à bancada, com o que mais o canônico ganhou depois da 8`},{versao:13,nome:`congregacaoNoPerfil`,oQue:`a congregação e o grupo de campo no Perfil do Kobi Note, que o relatório de campo assina`},{versao:14,nome:`documentoDeVenda`,oQue:`o documento de venda do Kobi Flow e as tabelas dos módulos do Perfex que vieram com ele`},{versao:15,nome:`textosDoUsuario`,oQue:`os textos dos documentos que o usuário do Kobi Flow escreve, ao lado dos de fábrica`}].map(e=>e.versao)),te(`nucleo`);function R(e,t,n,r,i){return{id:e,modulo_id:t,titulo:n,resumo:r,conteudo:i.map(e=>`<p>${e}</p>`).join(``),ordem:e,publicar:1}}R(1,``,`Visão geral`,`O que é o Kobi Note e como ele guarda suas coisas`,[`O Kobi Note é um aplicativo <strong>do seu aparelho</strong>. Tudo o que você escreve — anotações, metas, finanças, relatórios — fica gravado localmente, cifrado, e não é enviado a servidor nenhum. Para levar seus dados a outro aparelho, use o <strong>backup</strong>, no Sobre.`,`A tela inicial é o mapa do app: cada tile abre um módulo direto, e os tiles são agrupados em seções (Estudo Pessoal, Esteja Preparado, Extras, Pessoal, O Aplicativo). Não há tela intermediária. O botão de voltar do cabeçalho sempre sobe um nível.`,`Logo abaixo da agenda de hoje vem a <strong>faixa de números</strong> do mês: leitura, horas, estudo e metas. O cartão de metas é um botão — ele mostra as atingidas sobre o total e abre as Metas.`,`O tile <strong>Sincronizar</strong>, na seção O Aplicativo, baixa o <strong>conteúdo curado</strong> (guias, poesias, receitas, perguntas do jogo). Ele nunca apaga o que é seu, e o app funciona sem rede: sincronizar só atualiza o acervo.`,`Os botões do alto de toda tela abrem, de qualquer lugar, o seu cartão de contato e a ficha de emergência, e trocam o idioma e o tema claro ou escuro.`,`A ajuda de cada módulo mora aqui no Tutorial: um tópico por módulo, na ordem da tela inicial.`]),R(2,`anotacoes`,`Anotações`,`Escrever, organizar em pastas e apresentar`,[`Serve para tudo que você escreve: reuniões, discursos, visitas, estudos. Toque em <strong>+</strong> e escolha um modelo — ele já entra com os títulos prontos.`,`A gravação é automática: o texto é salvo pouco depois de você parar de digitar, e o rodapé do editor mostra a hora do último salvamento. Sem título, nada é gravado.`,`Dicas: <strong>fixe</strong> a anotação que você abre toda semana; <strong>arquive</strong> em vez de excluir o que já passou; e use o modo <strong>apresentar</strong> para ler no palco — o texto rola sozinho e a velocidade é ajustável.`,`Para tirar uma anotação do arquivo, filtre por <strong>Arquivadas</strong>, abra a anotação e toque de novo no botão do arquivo.`]),R(3,`guias`,`Guias Táticos`,`Ler os guias do acervo e escrever os seus`,[`Os guias são explicações prontas, que vêm no acervo curado. A lista mistura os do acervo com os seus; os seus aparecem com a marca <em>Minha</em>.`,`No guia aberto você pode ouvir em voz alta, apresentar com rolagem automática ou compartilhar o texto. Editar e excluir só valem para os seus.`,`Dica: toque em <strong>+</strong> para escrever um guia próprio — ele fica no aparelho e nunca é substituído por uma sincronização.`]),R(4,`poesia`,`A Verdade, em Poesia`,`Ler o acervo, favoritar e escrever`,[`O acervo de poesias é curado; as suas entram pelo <strong>+</strong> e ficam misturadas na mesma lista, com a marca <em>Minha</em>.`,`Na leitura há navegação sequencial (anterior/próxima) pelo acervo, barra de progresso, leitura em voz alta e modo apresentação.`,`Dica: a <strong>estrela</strong> marca favoritos, e favoritos sobem para o topo da lista. O botão de informação, na lista, explica o propósito das obras.`]),R(5,`receitas`,`Receitas`,`Buscar por categoria e guardar as suas`,[`A lista traz o acervo curado e as suas receitas. Os chips de emoji filtram por categoria — toque de novo no chip aceso para ver todas.`,`Ingredientes e modo de preparo são campos de <strong>linhas</strong>: um item por linha. É o formato que fica legível na leitura e na hora de ouvir em voz alta.`,`Dica: aqui você pode editar até as receitas do acervo — a sua versão fica no aparelho.`]),R(6,`jogo`,`Você se lembra?`,`Praticar com as perguntas do acervo`,[`É um jogo de perguntas para fixar o que você estuda. Cada partida tem dez perguntas sorteadas do acervo curado, que aumenta a cada sincronização.`,`Cada acerto rende <strong>XP</strong>. O que você já ganhou define o seu nível; o saldo se gasta em dicas, quando a pergunta aperta.`,`Dica: erre sem medo — a explicação de cada resposta é a parte que ensina. No fim da partida, dá para anotar o resultado no Caderno de Estudo.`]),R(7,`criacao`,`Teve um Projeto?`,`Puzzles sobre o projeto na natureza`,[`Cada módulo apresenta um ser vivo e o que a engenharia aprendeu dele. Você mexe nos controles do gráfico e vê o efeito na hora.`,`Dica: a ilustração de cada módulo ensina o que o puzzle não mostra — a conta, o mecanismo, o contexto. Vale abrir.`]),R(8,`entenda`,`Entenda Melhor`,`Assuntos explicados com gráficos`,[`Assuntos que rendem mais quando visualizados: escalas, medições, cronologias. Os controles ficam dentro do próprio gráfico, e os textos mudam conforme você mexe.`,`Dica: as referências ficam no fim de cada assunto — vale conferir a fonte.`]),R(9,`cronologia`,`Cronologia`,`A linha do tempo dos períodos bíblicos`,[`A linha do tempo vai da criação aos nossos dias, dividida em eras, com três trilhas: <strong>Bíblica</strong>, <strong>Mundial</strong> e <strong>Histórica</strong>. Os chips do alto ligam e desligam cada trilha, e o cabeçalho diz em que era você está.`,`Dica: use junto do Caderno de Estudo — datar o que você lê ajuda a fixar.`]),R(10,`caderno`,`Caderno de Estudo`,`Tudo o que você escreveu nos módulos de estudo`,[`O que você escreve nos módulos de estudo vem para cá: o Reflexo no Espelho do Imite, as reflexões dos Princípios, a sua resposta nas Respostas Frequentes, a pérola do Estudo Pessoal e o resultado de uma partida do Você se lembra?. Cada anotação guarda de onde veio.`,`A lista filtra por <strong>origem</strong> e pela busca. Pelo <strong>+</strong> você escreve uma anotação avulsa, sem módulo por trás.`,`Dica: releia de tempos em tempos. O que você escreveu meses atrás mostra o quanto o estudo andou.`]),R(11,`prep`,`Esteja Preparado`,`Kits, estoque e documentos`,[`O preparo para emergências: listas de verificação (kits), estoque de alimentos com validade e um cofre de documentos. Os três abrem de tiles próprios, na seção "Esteja Preparado" da tela inicial, que começa com o índice de prontidão.`,`O cofre guarda os documentos <strong>cifrados</strong> no próprio aparelho, e só abre com a <strong>biometria</strong> (a chave de acesso do aparelho). Ele se tranca de novo quando você sai da tela. Não há senha de reserva: perder a chave de acesso é perder o cofre.`,`Os <strong>Guias</strong> ficam na mesma seção: explicações do acervo curado, mais as que você escrever.`,`Dica: comece pelo kit sugerido e marque o que já tem; o indicador de prontidão mostra o quanto falta.`]),R(12,`financeiro`,`Financeiro`,`Entradas, saídas e recorrências`,[`Lançamentos de receita e despesa por categoria, com recorrências para o que repete todo mês. O painel mostra o saldo <strong>realizado</strong> e, ao lado, o <strong>previsto</strong>, que conta também o que ainda está em aberto.`,`A lista de lançamentos abre nas <strong>Vencidas</strong>. <strong>A vencer</strong> mostra o que está por vir; <strong>Pagas</strong> e <strong>Todas</strong> olham o mês escolhido.`,`Dica: crie as categorias que você realmente usa e apague as que não usa — relatório com categoria vazia não informa nada.`]),R(13,`metas`,`Minhas Metas`,`Objetivos com progresso`,[`Metas com prazo e progresso. Elas não têm tile próprio: quem as abre é o cartão de metas da faixa de números da tela inicial, o que mostra as atingidas sobre o total.`,`As metas <strong>automáticas</strong> calculam o progresso sozinhas, a partir do que você já registra: horas e estudos do Ministério, sessões do Estudo Pessoal, capítulos lidos, poesias lidas, perguntas do Você se lembra? e o Financeiro — sempre dentro do prazo da meta.`,`Dica: meta boa é a que caber num número — horas, capítulos, visitas.`]),R(14,`ministerio`,`Vida e Ministério`,`Relatório do mês e contadores`,[`O contador do mês, o relatório e os estudos bíblicos. O contador soma durante o mês, e o relatório fecha o mês com o total — inclusive o de um mês que já passou, escolhido no formulário.`,`O relatório é enviado pelo <strong>compartilhar</strong> e abre com o seu nome, congregação e grupo, que vêm do Perfil. Cada estudo tem uma linha do tempo com onde vocês pararam.`,`O atalho <strong>Quero fazer mais</strong> transforma uma meta de horas num compromisso semanal — inclusive os 15 horas do pioneiro auxiliar nos meses de arranjo especial.`,`Dica: registre no fim de cada saída, não no fim do mês — é a diferença entre um número exato e um número lembrado.`]),R(15,`estudo`,`Estudo Pessoal`,`Sessões de estudo com tempo`,[`Uma sessão tem tipo, assunto e um tempo de alvo, de 5 a 50 minutos. O cronômetro mede o tempo real, mesmo com o aparelho em outra tela.`,`No fim vem a <strong>pérola</strong>: o que você quer guardar da sessão. Ela vira uma anotação (na pasta Pérolas) e/ou uma entrada do Caderno de Estudo.`,`Dica: uma sessão curta registrada vale mais que uma longa esquecida.`]),R(16,`leitura`,`Leitura da Bíblia`,`Acompanhar a leitura por livro`,[`Acompanha a leitura capítulo por capítulo, nos 66 livros, em quatro visões: a ordem da Bíblia, a ordem cronológica, por escritor e o roteiro da Celebração. O app lembra a última visão que você usou, e o progresso vai junto no backup.`,`Dica: o progresso é por capítulo — marcar assim que terminar mantém o histórico honesto.`]),R(17,`calendario`,`Calendário`,`Eventos por tipo`,[`Eventos coloridos por tipo (reunião, compromisso, consulta, pessoal), em cinco vistas: dia, semana, mês, ano e agenda.`,`Os três cartões do topo falam do período que está na tela: as <strong>horas</strong> somadas, os <strong>conflitos</strong> (eventos com hora marcada que se sobrepõem) e os dias até o <strong>próximo especial</strong> — este contado de hoje, e não do período.`,`Dica: os tipos padrão já vêm criados; você pode criar os seus e reaproveitar em cada evento. É no cadastro de tipos que se marca qual deles é <em>especial</em>.`]),R(18,`perfil`,`Perfil / ICE — In Case of Emergency`,`Seus dados e o cartão de emergência`,[`Guarda os seus dados, incluindo as informações de emergência (ICE) e a recusa de sangue. Na seção de diretrizes há a data em que o cartão DPA físico foi atualizado e assinado — é dela que sai o aviso de renovação, um ano depois.`,`Dica: preencha os contatos de emergência mesmo que pareça improvável precisar — é a tela que outra pessoa vai abrir no seu lugar.`]),R(19,`tutorial`,`Tutorial do Kobi Note`,`Este próprio guia`,[`Este módulo. O índice abre com a visão geral e o Guia do Usuário, e traz depois um tópico por módulo, na ordem da tela inicial.`,`O conteúdo é curado: ele melhora a cada sincronização, sem precisar atualizar o app.`]),R(20,`imite`,`Imite a Sua Fé`,`Enxergar a boa intenção por trás do atrito`,[`Cada cartão traz um atrito comum da convivência e o julgamento apressado que ele provoca. Você escolhe entre três <strong>lentes</strong> qual explica melhor a intenção do outro.`,`Errar não custa nada: cada lente errada tem uma resposta própria, e é ali que está o ensino. Depois de acertar, aparecem o princípio bíblico, os textos e a galeria de quem mais fez aquilo na Bíblia.`,`Dica: a última parte de cada cartão é o <strong>Reflexo no Espelho</strong> — uma pergunta sobre você. O que você escreve ali vai para o Caderno de Estudo, e reler essas respostas meses depois vale mais do que o cartão em si.`]),R(21,`principios`,`Princípios Bíblicos`,`O que a Bíblia diz sobre as decisões do dia a dia`,[`É a parte de <strong>consulta</strong> do app: cada princípio traz a frase que o resume, os textos que o sustentam, por que ele existe e como ele se parece num dia comum. Não há exercício, não há acerto e não há erro — quem abre este módulo já sabe o que veio procurar.`,`A lista é filtrada por <strong>área da vida</strong> (família, trabalho, dinheiro, o que sai da boca...) e pela busca, que varre o título, a frase e os textos citados. A estrela guarda o princípio na faixa de favoritos, no alto da lista.`,`Dica: o fim de cada princípio traz perguntas de reflexão. O que você escreve ali vai para o Caderno de Estudo, junto com o que você escreveu no “Imite a Sua Fé” — é o mesmo caderno.`]),R(22,`faq`,`Respostas Frequentes`,`Treine a responder o que perguntam a você`,[`Não é um repositório de respostas prontas: é <strong>treino</strong>. Cada pergunta abre com a cena em que ela aparece — na porta, no trabalho, na mesa de domingo — e com a resposta que vem primeiro à cabeça, aquela que geralmente encerra a conversa em vez de abri-la.`,`Depois você escolhe entre <strong>lentes</strong>: os ângulos por onde dá para responder. Errar não custa nada — é nas lentes que não servem que está o ensino, e você pode tentar de novo. Acertando, aparecem a resposta pela Bíblia, os textos que a sustentam, quem na Bíblia respondeu assim e o link do artigo no jw.org, para ler o assunto na fonte.`,`A lista é filtrada por <strong>categoria</strong> (crenças, organização, pregação, vida familiar...) e pela busca. A estrela guarda a pergunta na faixa de favoritos, e a barra do alto mostra quantas você já fez.`,`Dica: o fim de cada pergunta pede a sua resposta, com as suas palavras. O que você escreve ali vai para o Caderno de Estudo — o mesmo do “Imite a Sua Fé” e dos “Princípios Bíblicos” —, e é o que dá o selo da pergunta.`]);const z=[{tabela:`not_guias`,colunas:[`titulo`,`texto`]},{tabela:`not_guias_local`,colunas:[`titulo`,`texto`]},{tabela:`not_poesias`,colunas:[`titulo`,`conteudo`]},{tabela:`not_poesias_local`,colunas:[`titulo`,`conteudo`]},{tabela:`not_receitas`,colunas:[`titulo`,`categoria`]},{tabela:`not_receitas_local`,colunas:[`titulo`,`categoria`]},{tabela:`not_cronologia`,colunas:[`titulo`,`resumo`,`referencia`]},{tabela:`not_principios`,colunas:[`titulo`,`principio`,`referencia`]},{tabela:`not_faq`,colunas:[`titulo`,`referencia`]}],ke=z.flatMap(e=>I(e)),Ae=L(z),B=[{versao:1,oQue:`a congregação e o grupo de campo no Perfil, que o relatório de campo assina`,aplicar:e=>{he(e,`not_meu_perfil`,[[`congregacao`,`TEXT NOT NULL DEFAULT ''`],[`grupo`,`TEXT NOT NULL DEFAULT ''`]])}}],je=Math.max(...B.map(e=>e.versao));function Me(e){let t=Number(e.escalar(`PRAGMA user_version`)??0),n=B.filter(e=>e.versao>t);n.length!==0&&e.emTransacao(()=>{for(let t of n)t.aplicar(e);e.executar(`PRAGMA user_version = ${je}`)})}function Ne(e){return`busca:${e}`}const Pe={...Object.fromEntries(z.map(({tabela:e})=>{let t=F(e);return[Ne(e),`SELECT rowid AS id, bm25(${t}) AS relevancia
         FROM ${t}
        WHERE ${t} MATCH ?
     ORDER BY relevancia
        LIMIT 200`]})),metasFontesNoPeriodo:`WITH janela(inicio, fim, mes_inicio, mes_fim, categoria_id) AS (SELECT ?, ?, ?, ?, ?)
                         SELECT
                           (SELECT COALESCE(SUM(t.valor), 0) FROM not_transacoes t, janela j
                             WHERE t.tipo = 0 AND t.data_vencimento >= j.inicio AND t.data_vencimento < j.fim) AS receitas,
                           (SELECT COALESCE(SUM(t.valor), 0) FROM not_transacoes t, janela j
                             WHERE t.tipo = 1 AND t.data_vencimento >= j.inicio AND t.data_vencimento < j.fim) AS despesas,
                           (SELECT COALESCE(SUM(t.valor), 0) FROM not_transacoes t, janela j
                             WHERE t.tipo = 1 AND t.categoria_id > 0 AND t.categoria_id = j.categoria_id
                               AND t.data_vencimento >= j.inicio AND t.data_vencimento < j.fim) AS despesa_categoria,
                           (SELECT COALESCE(SUM(r.horas), 0) FROM not_relatorios_ministerio r, janela j
                             WHERE r.ano * 100 + r.mes BETWEEN j.mes_inicio AND j.mes_fim) AS horas,
                           (SELECT COALESCE(SUM(c.revisitas), 0) FROM not_contadores_ministerio c, janela j
                             WHERE c.ano * 100 + c.mes BETWEEN j.mes_inicio AND j.mes_fim) AS revisitas,
                           (SELECT COUNT(*) FROM not_estudos_biblicos) AS estudos,
                           (SELECT COALESCE(SUM(s.duracao_minutos), 0) FROM not_sessoes_estudo s, janela j
                             WHERE s.concluido_em >= j.inicio AND s.concluido_em < j.fim) AS foco_min,
                           (SELECT COALESCE(SUM(s.duracao_minutos), 0) FROM not_sessoes_estudo s, janela j
                             WHERE s.tipo_ciclo = 'leitura'
                               AND s.concluido_em >= j.inicio AND s.concluido_em < j.fim) AS foco_leitura_min,
                           (SELECT COUNT(*) FROM not_sessoes_estudo s, janela j
                             WHERE s.concluido_em >= j.inicio AND s.concluido_em < j.fim) AS sessoes,
                           (SELECT COUNT(DISTINCT p.pergunta_id) FROM not_jogo_respostas p, janela j
                             WHERE p.respondida_em >= j.inicio AND p.respondida_em < j.fim) AS perguntas`,categoriasEmOrdem:`SELECT * FROM vw_note_categorias_financeiro`,eventosDoDia:`SELECT count(*) AS total
                   FROM not_calendario_eventos
                  WHERE data_inicio_epoch <= ? AND data_fim_epoch >= ?`};function Fe(e){let t=Pe[e];if(t===void 0)throw Error(`consulta desconhecida: ${e} (ver src/db/consultas.ts)`);return t}const Ie={perguntas:{tabela:`not_perguntas`,familia:`curado`},poesias:{tabela:`not_poesias`,familia:`curado`,apelidos:{anteriorId:`anterior_id`,proximoId:`proximo_id`}},receitas:{tabela:`not_receitas`,familia:`curado`},guias:{tabela:`not_guias`,familia:`curado`},kits:{tabela:`not_kits`,familia:`curado`,filhos:[{campo:`itens`,tabela:`not_kits_itens`,chave:`kit_id`}]},criacao_modulos:{tabela:`not_criacao_modulos`,familia:`curado`},imite_cartoes:{tabela:`not_imite_cartoes`,familia:`curado`},principios:{tabela:`not_principios`,familia:`curado`},faq:{tabela:`not_faq`,familia:`curado`},cronologia:{tabela:`not_cronologia`,familia:`curado`},anotacao_modelos:{tabela:`not_anotacao_modelos`,familia:`curado`},estoque_catalogo:{tabela:`not_estoque_catalogo`,familia:`curado`},tutorial:{tabela:`not_tutorial`,familia:`curado`},pasta:{tabela:`not_pasta`,familia:`privado`},anotacao:{tabela:`not_anotacao`,familia:`privado`},not_caderno_estudo:{tabela:`not_caderno_estudo`,familia:`privado`},sessoes_estudo:{tabela:`not_sessoes_estudo`,familia:`privado`},categorias_financeiro:{tabela:`not_categorias_financeiro`,familia:`privado`},transacoes:{tabela:`not_transacoes`,familia:`privado`},recorrencias_financeiro:{tabela:`not_recorrencias_financeiro`,familia:`privado`},meta:{tabela:`not_meta`,familia:`privado`},kits_local:{tabela:`not_kits_local`,familia:`privado`,filhos:[{campo:`itens`,tabela:`not_kits_local_itens`,chave:`kit_id`}]},documentos_cofre:{tabela:`not_documentos_cofre`,familia:`privado`},estoque_alimentos:{tabela:`not_estoque_alimentos`,familia:`privado`},relatorios_ministerio:{tabela:`not_relatorios_ministerio`,familia:`privado`},contadores_ministerio:{tabela:`not_contadores_ministerio`,familia:`privado`},estudos_biblicos:{tabela:`not_estudos_biblicos`,familia:`privado`},estudo_registros:{tabela:`not_estudo_registros`,familia:`privado`},meu_perfil:{tabela:`not_meu_perfil`,familia:`privado`},calendario_tipos:{tabela:`not_calendario_tipos`,familia:`privado`},calendario_eventos:{tabela:`not_calendario_eventos`,familia:`privado`},poesias_local:{tabela:`not_poesias_local`,familia:`privado`},guias_local:{tabela:`not_guias_local`,familia:`privado`},receitas_local:{tabela:`not_receitas_local`,familia:`privado`},jogo_progresso:{tabela:`not_jogo_progresso`,familia:`privado`},jogo_respostas:{tabela:`not_jogo_respostas`,familia:`privado`}};let V;function H(){if(V===void 0)throw Error(`o repositório ainda não foi montado`);return V}function Le(e){V=new ge(e,Ie),V.lerEsquemaAplicado()}function U(){return H().tabelas()}function Re(e,t,n){let r=S(t,H().colunasDa(t),n);e.executar(r.sql,r.parametros)}function ze(e,t,n,r){switch(t.op){case`otimizar`:return n&&(e.executar(`PRAGMA optimize`),e.executar(`PRAGMA incremental_vacuum(64)`)),n;case`pronto`:return n;case`todos`:return H().todos(t.store);case`consulta`:return e.consultar(Fe(t.nome),[...t.parametros??[]]);case`obter`:return H().obter(t.store,t.id);case`contar`:return H().contar(t.store);case`salvar`:{let e=H().salvar(t.store,t.registro);return r(),e}case`excluir`:return H().excluir(t.store,t.id),r(),null;case`substituirTudo`:return H().substituirTudo(t.store,t.registros),r(),null}}function Be(e){let t=e.consultar(`PRAGMA table_info(not_faq)`).map(e=>String(e.name));t.length===0||t.includes(`lentes`)||e.emTransacao(()=>{e.executar(`DROP TABLE IF EXISTS not_faq_fts`),e.executar(`DROP TABLE not_faq`)})}const Ve=[`vw_note_metas_fontes`,`vw_note_despesa_por_categoria`];function He(e){for(let t of Ve)e.executar(`DROP VIEW IF EXISTS ${t}`)}function Ue(e){if(Number(e.escalar(`SELECT COUNT(*) FROM sqlite_schema WHERE type = 'table' AND name = 'not_dados'`)??0)===0){W(e);return}let t=/* @__PURE__ */ new Set;H().emTransacao(()=>{for(let n of H().stores()){let r=e.consultar(`SELECT dados FROM not_dados WHERE tabela = ? ORDER BY id`,[n]);for(let e of r){let r;try{r=JSON.parse(String(e.dados))}catch{t.add(`${n} (JSON ilegível)`);continue}H().salvar(n,r)}e.executar(`DELETE FROM not_dados WHERE tabela = ?`,[n])}for(let n of e.consultar(`SELECT DISTINCT tabela FROM not_dados`))t.add(String(n.tabela));t.size===0&&(e.executar(`DROP TABLE IF EXISTS not_dados`),e.executar(`DROP TABLE IF EXISTS not_seq`)),W(e)}),t.size>0&&console.warn(`migração da Fase 8: store(s) sem destino, mantidos em \`not_dados\`: ${[...t].join(`, `)}. A tabela antiga não foi apagada.`)}function W(e){e.executar(`INSERT INTO not_meta_banco (chave, valor) VALUES (?, ?) ON CONFLICT(chave) DO NOTHING`,[`esquema_em_colunas`,`1`])}const G=`note.sqlite`,K=`dump`;let q,J=!1,Y;function X(){if(q===void 0)throw Error(`banco ainda não abriu`);return q}let We;function Ge(){return We??=new Promise((e,t)=>{let n=indexedDB.open(`note_fallback`,1);n.onupgradeneeded=()=>n.result.createObjectStore(`kv`),n.onsuccess=()=>e(n.result),n.onerror=()=>t(n.error??/* @__PURE__ */ Error(`IndexedDB não abriu`))}),We}async function Ke(e){let t=await Ge();return new Promise((n,r)=>{let i=t.transaction(`kv`,`readonly`).objectStore(`kv`).get(e);i.onsuccess=()=>n(i.result),i.onerror=()=>r(i.error??/* @__PURE__ */ Error(`leitura do IndexedDB falhou`))})}async function Z(e,t){let n=await Ge();return new Promise((r,i)=>{let a=n.transaction(`kv`,`readwrite`);a.objectStore(`kv`).put(t,e),a.oncomplete=()=>r(),a.onerror=()=>i(a.error??/* @__PURE__ */ Error(`gravação no IndexedDB falhou`))})}async function qe(){return await Ke(K)}async function Je(e){return Z(K,e)}function Ye(e){if(e===void 0||(e.tamanhoDe(e.caminhoDoBanco)??0)===0)return null;let t=e.abrir(e.caminhoDoBanco,{criar:!1,truncar:!1,exclusivo:!1});if(t===null)return null;let n=/* @__PURE__ */ new Uint8Array(16);return t.ler(n,0),n}async function Xe(){let e;try{e=await qe()}catch{return}if(e===void 0||e.versao!==2)return;let t=new Set(U());X().emTransacao(()=>{for(let[n,r]of Object.entries(e.tabelas))if(t.has(n))for(let e of r)Re(X(),n,e)})}let Q;function Ze(){J||Q!==void 0||(Q=setTimeout(()=>{Q=void 0;let e={};for(let t of U())e[t]=X().consultar(`SELECT * FROM ${y(t)}`);Je({versao:2,tabelas:e}).catch(e=>{console.warn(`persistência de fallback falhou`,e)})},200))}function Qe(e){return self.isSecureContext?e instanceof DOMException&&e.name===`NoModificationAllowedError`?`instancia-dupla`:`indisponivel`:`origem-insegura`}function $e(){let e=X().escalar(`SELECT count(*) FROM sqlite_master WHERE name = 'sqlite_stat1'`);Number(e??0)===0&&X().executar(`ANALYZE`)}function et(){X().executarRoteiro(se(`note`).join(`
`)),X().executar(ce)}function tt(){if(X().executarRoteiro(ke.join(`
`)),!(X().consultar(`SELECT valor FROM not_meta_banco WHERE chave = ?`,[`busca_construida`]).length>0)){for(let e of Ae)X().executar(e);X().executar(`INSERT INTO not_meta_banco (chave, valor) VALUES (?, ?) ON CONFLICT(chave) DO NOTHING`,[`busca_construida`,`1`])}}async function nt(){let e;try{e=await Oe.preparar({pasta:`note-opfs`,banco:G}),J=!0}catch(e){Y=Qe(e),console.warn(`OPFS indisponível (${Y}); Bioma SQL em memória com despejo no IndexedDB.`,e)}let t=Ye(e),n=e===void 0?`sem-arquivo`:ue(t),r=e===void 0?void 0:await pe(Ke,Z);q=await Ce({modulo:fetch(Ee),caminho:e?.caminhoDoBanco??`/${G}`,...e===void 0?{}:{arquivos:e},...r===void 0||n===`em-claro`?{}:{chave:r}}),n===`em-claro`&&r!==void 0&&(console.info(`Kobi Note: cifrando o banco deste aparelho pela primeira vez.`),X().rechavear(r)),X().executar(`PRAGMA auto_vacuum = INCREMENTAL`),Be(X()),He(X()),X().executarRoteiro(ee(`note`).join(`;
`)),Me(X()),Le(X()),Ue(X()),J||await Xe(),tt(),et(),$e();let i={tipo:`pronto`,versaoSqlite:X().versao,persistente:J,...Y===void 0?{}:{motivo:Y}};self.postMessage(i)}const $=nt();self.addEventListener(`message`,e=>{let t=e.data;$.then(()=>{let e={seq:t.seq,ok:!0,valor:ze(X(),t.operacao,J,Ze)};self.postMessage(e)}).catch(e=>{let n={seq:t.seq,ok:!1,erro:String(e)};self.postMessage(n)})}),$.catch(e=>{let t={tipo:`falha`,erro:String(e)};self.postMessage(t)});